...
pub fn run() -> Result<()> {
    ...
    // Server URL overrides are honored on internal dev and OSS channels. First-party release
    // channels silently ignore `--server-root-url` / `--ws-server-url` /
    // `--session-sharing-server-url` (and their `WARP_*` env-var equivalents) so shipped builds
